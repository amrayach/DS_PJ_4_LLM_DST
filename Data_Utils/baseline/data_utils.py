  ...
  def get_dialog_examples(self, dataset):
    ...
    for dialog_idx, dialog in enumerate(dialogs):
      tf.logging.log_every_n(tf.logging.INFO, "Processed %d dialogs.", 1000,
                             dialog_idx)
      examples.extend(
          self._create_examples_from_dialog(dialog, schemas, dataset, examples_2))

    # save examples_2 as piclke file
    with open('test_examples.pkl', 'wb') as f:
      pickle.dump(examples_2, f)
...
# --bert_ckpt_dir
  # ./schema_guided_dst/cased_L-12_H-768_A-12
  # --dstc8_data_dir
  # ./DS_PJ_4/dstc8-schema-guided-dialogue-master/
  # --dialogues_example_dir
  # ./schema_guided_dst/dialouges_example_output/
  # --schema_embedding_dir
  # ./schema_guided_dst/schema_embed_output/
  # --output_dir
  # ./schema_guided_dst/ft_model/
  # --dataset_split
  # test
  # --run_mode
  # train
  ...
  def _create_examples_from_dialog(self, dialog, schemas, dataset, examples_2):
    ...
    for turn_idx, turn in enumerate(dialog["turns"]):
      ...
      if turn["speaker"] == "USER":
        ...
        for i in turn_examples:
          curr_example_2 = self._create_example_2(system_utterance, user_utterance, i)

          examples_2.append(curr_example_2)

    # save examples_2 as piclke file
    with open('examples_2.pkl', 'wb') as f:
        pickle.dump(examples_2, f)

    return examples

  def _create_example_2(self, system_utterance, user_utterance, example_1):
    curr_example = {}
    curr_example["system_utterance"] = 'system: ' + system_utterance
    curr_example["user_utterance"] = 'user: ' + user_utterance
    curr_example["service_name"] = example_1.readable_summary['service_name']
    curr_example["active_intent"] = example_1.readable_summary['active_intent']
    # merge dict example_1[0].readable_summary['slot_values_in_state'] with curr_example
    curr_example["slot_values_in_state"] = example_1.readable_summary['slot_values_in_state']
    curr_example["schema"] = example_1.service_schema.schema_json
    curr_example["example_id"] = example_1.example_id

    curr_example["requested_slots"] = example_1.readable_summary['requested_slots']
...
class InputExample(object):
  ...
  def readable_summary(self):
    ...
    requested_slots = [
        self.service_schema.get_slot_from_id(idx)
        for idx, s in enumerate(self.requested_slot_status)
        if s == STATUS_ACTIVE
    ]
